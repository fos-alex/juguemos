...
*Attach this to every Claude Design project. It covers release 0.1 only. The full product is described in [product-concept.md](product-concept.md), but only the screens listed here should be designed.*

---

## 1. What Juguemos is

Juguemos ("let's play") is a play coach for families in Buenos Aires with children aged 1 to 5. It knows the family: the kids and their ages, what they love, the pet, and the toys, called by the names the family uses at home. It uses that to answer one everyday question: *"¿Qué hacemos ahora?"* ("What should we do right now?")
...
These come from the five commitments in [constitution.md](constitution.md).
...
Juguemos is designed in and for Buenos Aires. Local inspiration is welcome if it's used lightly: plazas and their jacarandás, *la rayuela* chalked on the pavement, *la ronda*, crafts made from cardboard boxes, family Sundays, and the city's strong tradition of children's books and music.

Avoid postcard clichés such as tango, mate, the Obelisco, or the flag.

## 5. Language and voice

All interface copy is in **Rioplatense Spanish with *vos*** (*hacé*, *escondé*, *contame*, *probá*). Never use *tú*, Spain Spanish, or neutral "international" Spanish.

The voice is a warm, experienced friend who knows the family. Sentences are short, words are plain, and humour is light. The app is never preachy or clinical, and it doesn't use baby talk with parents.
...
Use this family in every design instead of placeholder text. Real names and real activity text are what make Juguemos feel like it knows the family, and that feeling is exactly what 0.1 tests.
...
| Screen | What it does |
|---|---|
| **Sign in** | One parent, one account. The sign-in method isn't decided yet, so keep it generic. |
| **Tell us about your family** | A large text box with the onboarding prompt, and the example above as a hint. Onboarding is text only in 0.1. Voice notes arrive in 0.2, so leave room for a microphone button next to the text box, but don't design the recording flow. |
...
| **Correction form** | A plain form with kids, pet, interests, and toys. It's the fallback, not the main path. |
| **Home** | The big *"¿Qué hacemos ahora?"* button is the heart of the app. *"Hora del cuento"* sits here too, as the second action. |
| **Activity** | One idea: title, time, why it fits now, what you need, steps, and easier and harder versions. *"Otra idea"* shows a different one. It must be readable at a glance. |
...
**Not in 0.1, so don't design these:** the toy box, voice recording, goals, activity categories and filters, weather, the moments journal, parenting tips, recaps, holidays, the partner invite, feedback after an activity, and an English interface.
...
- **Fonts.** Use free, openly licensed fonts that can be self-hosted, such as those on Google Fonts. They need full Spanish support: *á é í ó ú ñ ü*, *¿* and *¡*, and the dash (—) used for dialogue in stories. Use at most two families, to keep loading fast on mid-range phones.
- **Simple to build.** The app is built by hand in React with no component library. A small set of simple, reusable components beats many one-off layouts.
...
*Juguemos* is Spanish for "let's play". It's an invitation rather than the name of a tool. Trademark and domain checks are still pending, so treat the wordmark as a working version.

## 10. References

*To add: apps, brands, and books we like and dislike, each with a line on why.*
