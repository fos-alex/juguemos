...
- **Tell a story.** *"Hora del cuento"* offers plot options starring the kids, the pet, and their toys by family name. The options are cast by code from a tweakable probability distribution — who leads, whether the pet and a toy are in, which interest is the theme — and interest keywords under them write a story on that theme in one tap. Since JUG-157 there are two: a classic led by the kid the story is for, and one that proposes something new, with a way to ask for a story by voice under them. Each option appears as soon as it is written, so the first card is there while the rest are still coming. The parent picks one and reads it on a screen with large text that stays awake. Until the LLM provider is chosen, stories come from templates filled in by code.
...
- **Detailed family profiles.** The parents and what the kids call them, so stories can bring them in; what animal the pet is; and the kind of home, kept for suggesting activities later.
...
- **A livelier app.** Motion that plays: buttons that give, juegos that arrive step by step, and small one-off moments like petals when the timer ends and *Fin* drawn at the end of a story. Marks for common interests, drawn as the animals and toys kids love (dinosaurs, dragons, dogs, trains, cars), beside stories, over a story's title, on the kids' interests, and on the toys in the baúl, and a story wait that draws with a crayon, a pencil, or a pen depending on the kids' ages.
