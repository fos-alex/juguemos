 ...
 * @param {{ returnTo: string, error?: string }} options `returnTo` is the screen's own path
 */
export function useGoogleSignIn({ returnTo, error }) {
  ...
  return {
    ...
    start() {
      if (!request.busy) void request.run(() => signInWithGoogle(returnTo))
    },
  }
}
