...
- **Copy still needs a voice pass** in these places, marked `Voice pass pending` in code: Listo, Prefiero un formulario, Guardar, the line after six seconds of thinking, Empezar, the timer screen, Otras opciones, Modo noche and Modo día, all account-screen copy, and the voice note's lines (`VoiceNote.jsx`, and "Mandar de nuevo" on `2d`).
- **Out of scope for 0.1, so don't build it:** the toy box, goals, categories and filters, weather, the journal, tips, recaps, holidays, the partner invite, post-activity feedback, and an English interface.
