...
- **Data comes only from `src/api`.** It has one module per area (`auth.js`, `family.js`, `toys.js`, `activities.js`, `stories.js`, `admin.js`), all calling through `request` in `http.js`, re-exported by `index.js`. Screens work with the shapes in `api/types.js`, and the modules translate the API's shapes to them. `mock.js` holds only email verification. Screens never call `fetch` themselves, and the web hardcodes no data.
...
- **Copy still needs a voice pass** in these places, marked `Voice pass pending` in code: Listo, Prefiero un formulario, Guardar, the line after six seconds of thinking, Empezar, the timer screen, Otras opciones, Modo noche and Modo día, all account-screen copy, and all of the toy box, which the 0.2 design will also restyle.
- **Out of scope for 0.1, so don't build it:** voice recording (the mic is a placeholder), goals, categories and filters, weather, the journal, tips, recaps, holidays, the partner invite, post-activity feedback, and an English interface.
