...
- **Each option is its own call** (JUG-139). `options()` draws the castings and runs one short model call per casting with `Promise.allSettled`, each asking for one plot as JSON and asked once more when the answer doesn't parse. The screen is whatever answered, in casting order, as long as one did; only a screen where every call failed is an `UpstreamError`. Calls carry a `max_tokens`, and on OpenRouter they ask a reasoning model to skip its thinking.
