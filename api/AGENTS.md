...
src/
  ...
  <domain>/           one folder per domain: accounts/, families/, toys/, activities/, stories/, health/
    ...
    <domain>.schema.js      the domain's tables and relations, in Drizzle (the toy box's
                            are in toys/toys.schema.js, the templates' in catalog/)
  ...
  stories/            besides the usual files: stories.service.js is the facade that picks
                      ...
                      storytelling.js and casting.js are the pure logic, story-audit.js
                      records what was offered, picked and written, and prompts/ holds the
                      texts: storyteller.js the narrator's core, bands.js one fragment per
                      age band, moments.js one per moment of day, and compose.js the
                      system prompt the three are joined into
  ...
  db/                 the Drizzle client, schema.js (every domain's tables, for the client and drizzle-kit), the migration runner, the demo accounts' seeder
...
- **Code decides, and the prompt gets only what applies** (JUG-139). The system prompt is put together by `prompts/compose.js`: the narrator's core, one of the eight age-band fragments in `prompts/bands.js`, and one of the two moment fragments in `prompts/moments.js`. The band is the youngest kid playing with a known age (no ages: band 3) and the moment is the Buenos Aires clock; the weather joins as a third fragment in 0.3. No band or moment a story isn't for is ever sent.
- **The bands are six months wide under four years** (JUG-145): `1`, `1.5`, `2`, `2.5`, `3`, `3.5`, then `4` and `5`, a year each. A kid of 1 and one of 1 and 10 months listen to different stories, so a kid's age is kept in months, `kids.age_months`, and counted from the day the parent gave it. Each band asks for more than the kid can say: a story slightly hard to follow beats one that is too simple, and the narrator's core says so.
- **The casting is drawn before the model is asked** (JUG-139). `casting.js` is pure: given the playing profile, the weights, a random function, and the castings of the last five stories, it draws who a story is about — whether the anchor kid is in, who leads, whether the pet and a toy are in and which, and which interest is the theme. `DEFAULT_WEIGHTS` is the one place the probabilities live, and the draw takes them as an argument so per-family weights can come later. The three castings of a screen differ, and one draw a screen makes one of them a wildcard, with none of the family's props. The random numbers and the weights stay on the casting, which is saved with the plot, with the story, and in the audit, so a draw can be read back.
- **The options are one call, read as it streams** (JUG-139). `options()` draws the three castings, hands them to one model call numbered `Trama 1` to `Trama 3`, and reads the answer as it arrives: `OptionsParser` in `storytelling.js` pulls each plot out of the `tramas` array as its braces close, so a plot is saved and sent on while the model is still writing the next. One call keeps the system prompt from being paid for three times. `GET /stories/options` is therefore a stream like `POST /stories/write`: one `option` event per plot, then `done`. The stale plots retire in the same transaction as the first new one, so a family is never left with none. An answer with no readable plot at all is asked once more, and only then is it an `UpstreamError`, which the family gets before the stream starts. The call carries a `max_tokens`, and on OpenRouter it asks a reasoning model to skip its thinking, without which the answer is all thought and no plots.
- **A keyword is a story with no plot** (JUG-140). `POST /stories/write` takes `{ keyword }` instead of `{ id }`, and the keyword has to be one of the interests of the kids playing (JUG-144), as typed, or it is a 400 with `UNKNOWN_KEYWORD`. Only the model writes these, so a server without one answers 503 with `LLM_OFF`. `generated.writeKeyword` draws one casting with the theme fixed to the keyword (`castKeyword`, never a wildcard) and asks for the story in a single call, with the `keywordStory` prompt: the model invents the plot and opens with a `TÍTULO:` line, which `StoryParser` hands back once so the stream can send a `title` event before the first paragraph. Without that line the title is built from the keyword. The story is saved with no plot and no template, its `keyword` column set, the band's longest minutes, and a teaser written by code. Every tap writes a new story; nothing is deduplicated.
- **The story audit** (JUG-139) is `story_audit`, written by `story-audit.js`: one row per option offered, one when a plot is picked, and one when its story is saved, each with the band, the mood, the casting, and the timings and word count in `details`. It holds no prompt text and no story text, and names the kids only by id. Nothing serves it over HTTP; it is read in SQL. A failed audit insert is logged and let go, since a story must never fail because its record didn't save.
- **A family from the parent's own words** (JUG-11). `POST /family/understanding` sends the text to the same LLM with `families/prompts/family.js` and returns a profile for the parent to confirm; it saves nothing. `families/understanding.js` keeps each name spelled as the parent wrote it and flags a kid or pet the text doesn't name. Each interest goes on the kid the text gives it to; the model's shared list, for what the text doesn't tie to one kid, goes on every kid (JUG-144). The text never goes into a log, and the model's answer can repeat it, so neither goes into an error message. It is stored only in `audit_transcripts`, and only while `AUDIT_TRANSCRIPTS` is on (JUG-116): `audit/audit.service.js` does nothing when it's off, and a voice note's transcription goes through the same `recordTranscript`. `/me` returns `familyFromText`, false without an LLM, so the web starts first run at the form; the endpoint itself answers 503 with `LLM_OFF` on a server that has none.
...
- **The catalog admin has no login yet** (JUG-109). Its routes under `/admin` are `catalog/catalog.routes.js` and `catalog.controller.js`, since what it edits is the catalog. They say `access: 'public'`, so `app.js` registers them only when `ADMIN_ENABLED` is true; otherwise they are a 404. Never turn it on where anyone outside the family can reach it, and remind Alex of the rule when work touches it.
...
- **Families:** each adult has one family for now, and the second parent joins in 0.6. The profile holds kids, pets, and toys, and each kid holds their own interests (`kid_interests`, JUG-144). A `Profile`'s own `interests` are derived, never stored: the interests of the kids in it, each once, so `playingProfile` carries only what the kids playing love and that is what stories and the `{interest}` slot use. The toy box (`toys/`, JUG-18) keeps the rest of what is known about each toy, and the profile knows toys only by name: saving it changes a toy's name and order and nothing else, so the form must send each toy's id.
- **The catalog owns every template** (JUG-125). `catalog/catalog.schema.js` holds `activity_templates` and `story_templates`, and `catalog.service.js` is the only place either table is read or written: the seed loads templates through its `load`, the admin creates and edits the activity ones, and activities and stories ask it for the templates they can offer, each with the texts its slots are in. Activities keeps `suggest` and its own table, and stories keeps the stories and the plots.
...
- **Data comes from the database, loaded by seeds,** never from application code or the web. No placeholder records or side effects that exist only to have data.
  - **Catalog templates** are in `seeds/catalog/` and load on every `docker compose up`, right after the migrations, through the catalog service's `load`. A template already in the database is never overwritten: the database is the catalog's home (JUG-9). Once loaded, a template changes in the admin. One deleted there keeps its row, marked `deleted_at`, so the seed still skips its slug and never brings it back.
