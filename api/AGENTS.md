...
npm run migration:generate -w api -- --name add-goals       # after changing src/db/schema/: writes the SQL migration
...
src/
  ...
  auth/               Better Auth: the instance, its /auth/* routes, the session preHandler
  db/                 the Drizzle client, the schema (schema/, one file per domain), the migration runner, the seeder
...
- **Better Auth's tables are ours:** they are defined in `db/schema/auth.js` with the rest of the schema, with plural names like every table, and Better Auth reads and writes them through its Drizzle adapter. A Better Auth upgrade or plugin that needs new columns adds them there and generates a migration.

## Migrations and seeds

- **The schema is code, and migrations are generated from it.** Change `src/db/schema/`, then run `npm run migration:generate -w api -- --name <name>`: drizzle-kit compares the schema with the last snapshot in `migrations/meta/` and writes the SQL. Read that SQL before committing it, since a renamed column can come out as a drop and an add. For what the schema can't express, such as moving data, `--custom` writes an empty migration to fill in. Commit the SQL and `meta/` together. A test fails when the migrations don't match the schema.
...
4. If the schema changes, change `src/db/schema/` and generate a migration. If exploring the app needs data, add it to the seeds.
