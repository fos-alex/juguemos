...
const profile = (overrides) => ({
  id: 'family',
  name: null,
  kids: [{ id: 'k1', name: 'Milán', ageMonths: 26, playing: true, interests: ['los dinosaurios'] }],
...
test('ages fit by the month, ends included, and an unknown age fits anything', () => {
  const toddlers = { minAgeMonths: 12, maxAgeMonths: 36 }
  assert.equal(fitsAge({ ageMonths: 12 }, toddlers), true)
  assert.equal(fitsAge({ ageMonths: 36 }, toddlers), true)
  assert.equal(fitsAge({ ageMonths: 37 }, toddlers), false)
  // A kid of eleven months is too young for a range that starts at twelve.
  assert.equal(fitsAge({ ageMonths: 11 }, toddlers), false)
  assert.equal(fitsAge({ ageMonths: null }, toddlers), true)
})
...
test('a template is filled only when the family has what it needs', () => {
  ...
  const noInterests = [{ id: 'k1', name: 'Milán', ageMonths: 26, playing: true, interests: [] }]
  assert.equal(fillFor(profile({ kids: noInterests, interests: [] }), template(['{interest}']), random), null)
  const tooOld = [{ id: 'k', name: 'Sofi', ageMonths: 72, playing: true, interests: [] }]
...
test('everyKid needs the whole family in the age range', () => {
  const kids = [
    { id: 'k1', name: 'Milán', ageMonths: 12, playing: true, interests: [] },
    { id: 'k2', name: 'Sofi', ageMonths: 52, playing: true, interests: [] },
  ]
...
test('the kid in the slot is the first one in the age range', () => {
  const kids = [
    { id: 'k1', name: 'Sofi', ageMonths: 72, playing: true, interests: [] },
    { id: 'k2', name: 'Milán', ageMonths: 26, playing: true, interests: [] },
  ]
...
test('{interest} is something the kid in {kid} loves, so that kid has to love something', () => {
  const kids = [
    { id: 'k1', name: 'Milán', ageMonths: 26, playing: true, interests: [] },
    { id: 'k2', name: 'Sofi', ageMonths: 40, playing: true, interests: ['dibujar'] },
  ]
